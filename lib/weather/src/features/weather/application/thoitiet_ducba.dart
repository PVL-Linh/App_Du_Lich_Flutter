import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:complex_ui_rive_animation/weather/src/features/weather/data/weather_repository.dart';
import 'package:complex_ui_rive_animation/weather/src/features/weather/domain/forecast/forecast_data.dart';
import 'package:complex_ui_rive_animation/weather/src/features/weather/domain/weather/weather_data.dart';

final cityProvider_ducba = StateProvider<String>((ref) {
  return 'Quận 1 TPHCM';
});

final currentWeatherProvider =
    FutureProvider.autoDispose<WeatherData>((ref) async {
  final city = ref.watch(cityProvider_ducba);
  final weather =
      await ref.watch(weatherRepositoryProvider).getWeather(city: city);
  return WeatherData.from(weather);
});

final hourlyWeatherProvider =
    FutureProvider.autoDispose<ForecastData>((ref) async {
  final city = ref.watch(cityProvider_ducba);
  final forecast =
      await ref.watch(weatherRepositoryProvider).getForecast(city: city);
  return ForecastData.from(forecast);
});
